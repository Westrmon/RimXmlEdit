...
public static class XmlConverter
{
    ...
    {
        var rootElement = new XElement(rxStruct.IsPatch ? "Patch" : "Defs");
        var comment = new XComment("Automatically generated by RimXmlEdit");
        ...
        foreach (var genericInfo in rxStruct.Defs)
        {
            ...
            {
                if (!string.IsNullOrEmpty(genericInfo.Ref)) topLevelElement.SetAttributeValue("Class", genericInfo.Ref);
            }

            foreach (var field in genericInfo.Fields)
            {
                var fieldElement = BuildXmlElement(field);
                if (fieldElement != null) topLevelElement.Add(fieldElement);
            }
    ...
    {
        var rootElement = new XElement("ModMetaData");
        var comment = new XComment("Automatically generated by RimXmlEdit");
        rootElement.Add(comment);
        var doc = new XDocument(new XDeclaration("1.0", "utf-8", null), rootElement);
        foreach (var genericInfo in rxStruct.Defs)
    ...
    public static RXStruct? Deserialize(string xmlContent)
    {
        ...
        foreach (var topLevelElement in root.Elements())
        {
            ...
            if (!rxStruct.IsPatch)
            {
                ...
                genericInfo.IsAbstract =
                    bool.TryParse(topLevelElement.Attribute("Abstract")?.Value, out var isAbstract) && isAbstract;
                genericInfo.Name = topLevelElement.Attribute("Name")?.Value ?? string.Empty;
                genericInfo.IgnoreConfigErrors =
                    bool.TryParse(topLevelElement.Attribute("ignoreConfigErrors")?.Value, out var ignoreConfigErrors) &&
                    ignoreConfigErrors;
            }
            else
            {
            ...
            {
                if ((!rxStruct.IsPatch && fieldElement.Name.LocalName == "defName") ||
                    fieldElement.Name.LocalName == "name") genericInfo.Name = fieldElement.Value;
    ...
    private static XElement? BuildXmlElement(XmlFieldInfo field)
    {
        ...
        if (!string.IsNullOrEmpty(field.Ref)) element.SetAttributeValue("Class", field.Ref);
    ...
    private static XmlFieldInfo ParseXmlElement(XElement element)
    {
        ...
        var isList = element.HasElements && (element.Elements().All(e => e.Name.LocalName == "li") ||
                                             element.Elements().GroupBy(e => e.Name).Any(g => g.Count() > 1));

        if (isList)
        {
            var list = new List<XmlFieldInfo>();
            foreach (var child in element.Elements()) list.Add(ParseXmlElement(child));
            fieldInfo.Value = list;
        }
        else if (element.HasElements)
        {
            var dict = new Dictionary<string, XmlFieldInfo>();
            foreach (var child in element.Elements()) dict[child.Name.LocalName] = ParseXmlElement(child);
